...
const Audio = () => {
  ...
  const { data,isLiked,index } = location.state || {};
  const [liked, setLiked] = useState();
  const [indexs , setindexes] = useState(index)
// console.log(index);
...
// console.log(userData,'isloc');
 ...
 useEffect(() => {
  ...
  const getDownloadedAudios = () => {
    ...
    // Retrieve stored audios for the specific user
    const storedData = localStorage.getItem(`DownloadedAudios_${userId}`);
    const audios = storedData ? JSON.parse(storedData) : [];
  
    console.log("Fetched downloaded audios:", audios);
    setDownloadList(audios);
  };
  ...
  const handleDownload = async () => {
    ...
    console.log(
      "Download file name:",
      `${encodeURI(AudioPath)}${encodeURIComponent(selectedSubCategory.subdown_audio)}`
    );
    ...
    if (isAlreadyDownloaded) {
      successMessage('You have already downloaded this audio!')
      setProgress(0)
      // successMessage('this audio is already downloaded!')
      // handleImageClick();
    } else {
      // Create an audio object
      const audio = {
        audioDescription: selectedSubCategory.description,
        audioImage: selectedSubCategory.subcategory_img,
        audioTitle: selectedSubCategory.subCatName,
        audioFileName: `${encodeURI(AudioPath)}${encodeURIComponent(selectedSubCategory.subdown_audio)}`,
      };
  
      // Update the user's download list
      const updatedDownloadList = [...userDownloadList, audio];
  
      // Save to localStorage under the user-specific key
      try {
        localStorage.setItem(
          `DownloadedAudios_${userId}`,
  ...
  const getCourseLikes = async (vid,like) => {
    ...
    console.log(vid,'vid',vid._id,'liked',liked)
    // try {
    ...
    //   const response = await fetch(API_URL4004 + "like_CourseAudio", requestOptions);
  ...
  const getAudios = async () => {
    ...
    fetch(API_URL4004 + "getLikedAudio/" + user_id, requestOptions)
      ...
      .then((result) => {
        ...
        setSelectedSubCategory(result.response[index])
        setLiked(result.response[index].isLiked)
      })
      .catch((error) => console.error(error));
  };

  const getAudios1 = async (index1) => {
    const user_id=localStorage.getItem('user_id')
    console.log(user_id);
    ...
    fetch(API_URL4004 + "getLikedAudio/" + user_id, requestOptions)
      ...
      .then((result) => {
        console.log('New new',result.response)
        setSubCategories(result.response)
        setSelectedSubCategory(result.response[index1])
        setLiked(result.response[index1].isLiked)
      })
      .catch((error) => console.error(error));
  };
...
/>
  ...
  <div
    ...
    onClick={() => handleCardClick(item,index)} // Play selected audio and move it to the top
  >
