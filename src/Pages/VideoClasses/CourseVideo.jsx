...
export default function CourseVideo() {
  ...
  const {id,tid,vname,title,desc,lenimg,file,downloadFileName,audio_files,category_id,chunkdatavedio,Duration,intensity,likes,instructor_id,videoIds}=location.state || {}
  console.log('1010',likes);
  ...
  const [instructorVids,setInstructorvids]=useState([])
  ...
  const [likeId, setLikeId] = useState(chunkdatavedio[0]?._id);
  ...
  // console.log(instructor_id,'insse')
  ...
  const getInstructor = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL4004}get_instructor_course?instructor_id=${instructor_id}&user_id=${user_id}`, requestOptions);
      const result = await response.json(); // Parsing the response
      if(result.Status === true){
        console.log(result,'instructors');
      setInstructorvids(result.response)
  ...
  const getsubCourse = () =>{
    ...
    .then((result) => {
        console.log(result.data.courseData,'result.data',result.data)
        if(result.Status === true){
            setsubCourse(result.data.courseData)
        }
    })
  ...
  // const handleReportPlayback = ({ isDarkMode }) => {
  //   setShowSettings(false);
  
  //   // Dynamically determine the theme class based on props
  //   const themeClass = isDarkMode ? 'swal-dark-mode' : 'swal-light-mode';
...
setLikeId(selectedItem.chunk_data[0]?._id)
}
};
const handleMediaChange = (selectedItem) => {
  const categorytype = selectedItem?.categorytype ; // Default to 'video' if categoryType is not provided
  console.log(selectedItem.category_id,'selectedItemnoisi')
  if(isLocked && selectedItem.subscriptionType === 'Pro'){
    setModalOpen(true)
  }
...
const renderCourseContent = () => {
      ...
      <>
        ...
        {subCourse
          ...
          .map((item, itemIndex) => (
            <div
              key={item._id}
              className={`video-right-container-course03 ${item.subscriptionType === 'Pro' && isLocked ?  'pro-overlay-item-00' : ''}`}
              style={{
                backgroundColor: darkMode ? "#2C2C2E" : "rgba(236, 236, 236, 1)",
              }}
              onClick={() => handleVideoChange(item)} // Change video on click
            >
              <div className="side-videos03">
                <img src={ImagePath + item.subcategory_img} loading="lazy" alt="" />
              </div>
              <div className="right-midle03">
                <div className="video-text-00-03">
                  {`${itemIndex + 1}. ${item.subCatName}`}

                </div>
                
                <div className="video-time03">
                  {`${Math.floor(Math.round(item?.Duration) / 60)}:${Math.round(item?.Duration) % 60 < 10 ? '0' : ''}${Math.round(item?.Duration) % 60}`} min
                  </div>
    {item.subscriptionType==='Preview' && isLocked ?( <span className="pro-badge-cv">Preview</span>):item.subscriptionType==='Pro' && isLocked ?(<span className="pro-badge-cv-00">🔒</span>):('')}
  ...
  return (
    <div className='overall-container03'>
        <Toaster
        toastOptions={{
          style: { backgroundColor: '#222122', color: '#fff' },
        }}
      />
     <div  className='video-title03'>
      {instructor_id ? 
        <PageTitle title={instructorVids[0]?.instructor_data?.name} />
    :
     <PageTitle title={title1}/>
      }
        </div>
        <div className='top-items-continer03'>
...
<div  className='video-main-container03'>
  <div className='left-container03'>
        <div className='video-player03'
...
{vidPath && chunkdatavedio1.length > 0 && (
  ...
  backgroundSize: 'cover',
      ...
      >
        ...
        {/* <Player
            ...
            <div className='video-controls-container03'>
              ...
              <div className='video-controls-right03'>
                ...
                <div className='video-control-4-03' onClick={toggleSettings} style={{display: showControls ? 'flex' : 'none'}}>
                  <img src='/assets/videocontrol-3.png' loading="lazy" alt="" />
                </div>
                {showSettings && !showAudioSettings && !isPickerOpen && (
                  <div className='video-settings03'>
        ...
        {showAudioSettings && (
                  <div className='audio-settings03'>
                    <div className='title-container03'>
                      <div className='audio-title03'>Audio</div>
        ...
        {/* {instructor_id ? '': */}
        <div className='small-main-container03'>
          <div style={{display:'flex',flexDirection:'row',gap:'1rem',width:'50%'}}>
          <div className='small-div-103'
           >
            <div className='small-icon-103'>
              <img src='/assets/play-button.png' style={{filter:'invert(100%)'}} alt="play button" loading='lazy'/>
            </div>
            <div className="small-text-103"> {`${Math.floor(Math.round(duration) / 60)}:${Math.round(duration) % 60 < 10 ? '0' : ''}${Math.round(duration) % 60}`} min</div>
            </div>
          <div className='small-div-203'
            >
            <div className='small-icon-203'>
            <img src='/assets/Intencity.png' loading="lazy" alt="" />
            </div>
            <div className='small-text-203'>
              {/* {intensity.length > 6 ? intensity.slice(0, 6) + '...' : intensity} */}
              {intensity}
            </div>
   ...
   {liked ?
          <img
            ...
            :
             <img
              ...
              // style={{ width: '50px', height: '50px', marginRight: '20px', cursor: 'pointer',filter:'invert(100%)' }}
              onClick={() => getCourseLikes(likeId)}
            />
          }
            </div>
            {showImage && (
        <div className="fav-icon103" onClick={handleDownload}>
          <img
    ...
    text: {
      ...
      transform: 'translateY(0px)',  // Fine-tune vertical alignment if necessary
        ...
        {/* // } */}
        <div className='video-description03'>
        <div className='video-disc03'>{title1}</div>
        <div className='video-para03'>{desc1}</div>
        </div>
        </div>
        <div className='overall-videos03'>
          {instructor_id ? (
            <div>
               {instructorVids?.map((item,index)=>(
        <div>
        ...
        className={`video-right-container-course03 ${item.subscriptionType === 'Pro' && isLocked ?  'pro-overlay-item-00' : ''}`}
         style={{
          backgroundColor: darkMode ? "#2C2C2E" : "rgba(236, 236, 236, 1)",
               ...
               <div className='side-videos-03'>
      {/* <video src={vidPath+item.vid_name}></video>       */}
      <div 
      className='side-videos03'
      >
        <img src={ImagePath + item.subcategory_img} alt="" />
       </div>
       </div>
       <div className='right-midle03'>
  <div className='video-info-03'> {/* Wrap text and time */}
    <div className='video-text-00-03'>
      {item.subCatName}
    </div>
    <div className='video-time03'>{`${Math.floor(Math.round(item?.Duration) / 60)}:${Math.round(item?.Duration) % 60 < 10 ? '0' : ''}${Math.round(item?.Duration) % 60}`} min</div>
    {item.subscriptionType==='Preview' && isLocked ?( <span className="pro-badge-cv">Preview</span>):item.subscriptionType==='Pro' && isLocked ?(<span className="pro-badge-cv-00">🔒</span>):('')}

  </div>
</div>

       <div className='video-arrow-03'>
        ...
        <Modal isOpen={upgradeModal} onClose={handleUpgradeclose}>
          <div className='overall-upgrade-container03'>
            <div className='upgrade-icon03'>
              <img src='/assets/ic_round-workspace-premium.png' loading="lazy" alt="" />
            </div>
            <div className='upgrade-text-container03'>
              <div className='upgrade-title03'>
              UPGRADE TO <br /> PREMIUM
              </div>
              <div className='upgrade-subtext03'>
                When you subscribe,you'll get <br />instant unlimited access
              </div>
            </div>

            <div className='upgrade-btn-container03'>
              <div  className='upgrade-btn03' onClick={handlePremiumModalOpen}>Be Premium</div>
              <div className='up-cancel-btn03'>Cancel</div>
            </div>
        ...
        <div>
          <div className="premium-top-title03">BE PREMIUM</div>
          <div className="premium-title03">Get Unlimited <br /> Access</div>
          <div className="premium-text03">When you subscribe, you’ll get <br />
          instant unlimited access</div>

          <div >
            <div className='radio-container03'style={{backgroundColor: selectedOption==='monthly' ? '#FF5F6747':'#2c2c2e'}} >
              <input type="radio"
               name="option"
               id="radio1" 
              value='monthly'
               ...
               />
              <div className="radio-contents-container-103" for='radio1'>
                <div>
                  <div className="radio-head-103">Monthly</div>
                  <div className="radio-tail-103" style={{color: selectedOption === 'monthly' ? '#ff5f67':'#ffffff',}}>Pay monthly</div>
                </div>
              <div className="radio-right-container03">
                <div className="radio-right03">
                  <sup>$</sup>
                  19.99
                  <sub>/m</sub></div>
              </div>
            ...
            <div className='radio-container03'style={{backgroundColor: selectedOption==='yearly' ? '#FF5F6747':'#2c2c2e'}}>
              <input type="radio"
               name="option"
                id="radio2"
                 value="yearly"
               ...
               />
              <div className="radio-contents-container-203">
                <div>
                  <div className="radio-head-203">Yearly</div>
                  <div className="radio-tail-203" style={{color: selectedOption==='yearly' ?'#ff5f67':'#ffffff'}}>Pay for full year</div>
                </div>
              <div className="radio-right-container-203">
                <div className="radio-right-203">
                  <sup>$</sup>
                  129.99
                  <sub>/y</sub></div>
              </div>
        ...
        <div className="modal-premium-btn03">Subscribe Now</div>
